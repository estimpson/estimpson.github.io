...
const vsSource = `#version 300 es
        ...
        void main() {
            gl_Position = vec4(aPos, 1.0);  // we pass a vec3 to vec4 constructor along with 4th value
        }
    `;
...
const fsSource = `#version 300 es
        ...
        uniform vec4 ourColor; // we set this variable in javascript
...
class WebGLExperiment {
    ...
    initialize() {
        ...
        // to animate, we utilize Window.requestAnimationFrame() which takes a callback to render the scene
        requestAnimationFrame(this.render.bind(this)); // bind(this) allows access to class members
    }

    render() {
        this.drawScene(this.gl, this.programInfo, this.VAO); // do the drawing
    ...
    initRectangleVertexArrayObject(gl) {
        ...
        // 2.2. create array of positions for rectangle
        const vertices = [
    ...
    drawScene(gl, programInfo, vao) {
        ...
        const timeValue = Math.PI * (new Date()).getTime() / 1000.0;  // this creates a period of 2 seconds
        const greenValue = Math.sin(timeValue) / 2.0 + 0.5;
        const ourColorUniform = gl.getUniformLocation(programInfo.program, "ourColor");
        gl.uniform4f(ourColorUniform, 0.0, greenValue, 0.0, 1.0);
