...
class WebGLExperiment {
    ...
    initRectangleVertexArrayObject(gl) {
        ...
        // 3. copy our vertex indices in a buffer for OpenGL to use
        // 3.0. create a element buffer object for indices
        const ebo = gl.createBuffer();
        // 3.1. bind our element buffer object
        gl.bindBuffer(
            gl.ELEMENT_ARRAY_BUFFER, // used for indices
            ebo                      // the ID of the buffer
        );
        // 3.2. create array of indices for the triangles of the rectangle
        const indices = [
            0, 1, 3, // first triangle is top right -> bottom right -> top left
            1, 2, 3, // second triangle is bottom right -> bottom left -> top left
        ];
        // 3.3. send (static data, indicating the indeces don't change) data
        gl.bufferData(
    ...
    drawScene(gl, programInfo, vao) {
        ...
        // do the drawing
        {
            ...
            const indexCount = 6;
            // wireframe mode:
            gl.drawElements(gl.TRIANGLES, indexCount, gl.UNSIGNED_INT, offset);
        }
    }
